...
def get_node_star(graph):
	node_star_dict = dict()
	for node, node_label in graph.nodes('label'):
		out_edges = [(e_label, 1) for _, _, e_label in graph.edges(node, 'label')]
		in_edges = [(e_label, -1) for _, _, e_label in graph.in_edges(node, 'label')]
		star = tuple([node_label] + sorted(in_edges+out_edges))
...
def get_star_dict(graph_list):
	...
	for g in graph_list:
		...
		## compute all stars
		for u, label in g.nodes('label'):
			out_edges = [(e_label, 1) for _, _, e_label in g.edges(u, 'label')]
			in_edges = [(e_label, -1) for _, _, e_label in g.in_edges(u, 'label')]
			star = tuple([label] + sorted(in_edges+out_edges))
			node_star[u] = star
		...
		for u, v, label in g.edges(data='label'):
			star_dict[node_star[u]].add((node_star[v], label, -1)) ## -1 since edge is going inside v
			star_dict[node_star[v]].add((node_star[u], label,  1)) ## 1 since edge is going outside u
...
def get_star_embedding(star_list, node_label_embeddings, edge_label_embeddings, edge_feature_dim):
	...
	for star in star_list:
		...
		for edge_label, direction in star[1:]:
			aggregrated_edge_embedding += edge_label_embeddings[edge_label]*direction
...
def pack_batch(graphs, node_label_embeddings, edge_label_embeddings, star_embeddings, star_dict):
	"""Pack a batch of graphs into a single `GraphData` instance.
		Args:
			graphs: a list of generated networkx directed graphs.
		Returns:
	...
	for i, g in enumerate(graphs):
		...
		edge_directions = [(u, v) for u,v in g.edges()]
		...
		for i_edge in range(len(edges)):
			u, v = edges[i_edge]
			if (u,v) == edge_directions[i_edge]:
				v_direction = -1
			elif (v,u) == edge_directions[i_edge]:
				v_direction = 1
			else:
				raise ValueError('incorrect edges')
			star_u = node_star_dict[u]
			candidate_stars = [star_embeddings[star] for star, edge, direction in star_dict[star_u]
								if edge == edge_labels[i_edge] and direction == v_direction] ## new stars
			candidate_stars += [star_embeddings[node_star_dict[node-n_total_nodes]]
								...
								and (node_star_dict[node-n_total_nodes], edge_labels[i_edge], v_direction) in
								star_dict[star_u]] ## existing stars
...
def nx_to_json(graphs, output_file):
    sg_list = list()
    for g in graphs:
...
def json_to_nx(file):
	...
	for sg in sg_list:
		obj = sg['objects']
		rel = sg['relationships']
		g = nx.DiGraph()
		for u in range(len(obj)):
			g.add_node(u, label=obj[u])
		for l in rel:
