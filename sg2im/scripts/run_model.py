...
def main(args):
  ...
  batch_size = 5
  imgs = list()
  for i in range(0, len(scene_graphs), batch_size):
    with torch.no_grad():
      batch_imgs, boxes_pred, masks_pred, _ = model.forward_json(scene_graphs[i : i+batch_size])
    batch_imgs = imagenet_deprocess_batch(batch_imgs)
    imgs.append(batch_imgs)
  imgs = torch.cat(imgs, axis=0)

  # Save the generated images
  imgs_np = imgs.numpy()
  with open(os.path.join(args.output_dir, 'imgs.pkl'), 'wb') as f:
    pickle.dump(imgs_np, f)
