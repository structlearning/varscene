...
from sparse_scene_graphs_dataset import SparseSceneGraphsDataset
...
class Solver(object):
    ...
    def __init__(self, config):
        ...
        # Data loader.
        self.data = SparseSceneGraphsDataset()
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            if (i+1) % self.log_step == 0:
                a, x = self.data.next_validation_batch(self.batch_size)
                ## mols, _, _, a, x, _, _, _, _ = self.data.next_validation_batch()
                z = self.sample_z(a.shape[0])
                print('[Valid]', '')
            else:
                a, x = self.data.next_train_batch(self.batch_size)
    ...
    def test(self):
        ...
        with torch.no_grad():
            a, x = self.data.next_test_batch(64)
