...
class GenerateGraph():
    ...
    def __init__(self, z, first_star):
        ...
        for e_label in  first_star[1:]:
            ...
            self.parent_edge_dict[self.node_no] = (0, e_label)
    ...
    def get_candidate_star_features(self, star_dict, star_embeddings):
        ...
        parent, e_label = self.parent_edge_dict[node]

        ## compute candidate stars
        ## new stars
        self.candidate_stars = [star for star, edge in star_dict[self.node_star_dict[parent]] if e_label==edge]
        ...
        for existing_node in self.nodes_at_dist[self.dist_dict[parent]]:
            ...
            if (star, e_label) not in star_dict[self.node_star_dict[parent]]:
                continue
            for _, v, edge_label in self.G.edges(existing_node, data='label'):
    ...
    def sample_and_add_star(self, candidate_logits):
        ...
        # print('logits %s, scores %s, scores np %s' % (candidate_logits.size(), candidate_scores.size(), candidate_scores_numpy.shape))
        ...
        if self.candidate_stars_source[next_star_idx] is None:
            ...
            for e in  remaining_edges:
                ...
                self.parent_edge_dict[self.node_no] = (node, e)
