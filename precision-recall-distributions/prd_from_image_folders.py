...
parser.add_argument('--inception_path', type=str,
                    default='inception_weights/inception.pb',
...
def load_images_from_dir(directory, types=('png', 'jpg', 'bmp', 'gif')):
    ...
    with open(os.path.join(directory, 'imgs.pkl'), 'rb') as f:
        imgs = pickle.load(f)
...
if __name__ == '__main__':
    ...
    for directory in eval_dirs:
        ...
        if args.verbose:
            print('computing PRD')
        size = min(real_embeddings.shape[0], eval_embeddings.shape[0])
        prd_data.append(prd.compute_prd_from_embedding(
            eval_data=eval_embeddings[:size],
            ref_data=real_embeddings[:size],
    ...
    for directory, f_beta in zip(eval_dirs, f_beta_data):
        print('%.5f %.5f     %s' % (f_beta[0], f_beta[1], directory))
