    ...
    def _initialize_node_repr(self, g, node_type, graph_embed):
        ...
        hv_init = self.initialize_hv(
            torch.cat([
                ...
                graph_embed], dim=1))
        # print('1', g.nodes[num_nodes - 1].data)
        # g = g.to(device=self.device)
        g.nodes[num_nodes - 1].data['hv'] = hv_init#.cpu()
        g.nodes[num_nodes - 1].data['a'] = self.init_node_activation#.cpu()
        g.nodes[num_nodes - 1].data['label'] = torch.LongTensor([[node_type]]).to(device=self.device)#.cpu()
        # print('2', g.nodes[num_nodes - 1].data)
    ...
    def forward(self, g_list, training, a=None):
        ...
        for i, g in enumerate(g_list):
            ...
            if not stop:
                ...
                num_nodes = g.number_of_nodes()
                # print('3', g.nodes[num_nodes - 1].data)
                # g = g.to(device=self.device)
...
class AddEdge(nn.Module):
    ...
    def forward(self, g_list, training, a=None):
        ...
        # print('YO')
        # print(len(g_list))
        # print(g_list[0].nodes[g_list[0].number_of_nodes() - 1].data)
        batch_graph_embed = self.graph_op['embed'](g_list)
...
class DGMG(nn.Module):
    ...
    def prepare(self, batch_size, training):
        ...
        for i in range(batch_size):
            g = dgl.DGLGraph().to(device=self.device)
            g.index = i
